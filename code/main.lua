...
Main class for executing RNN, GRU and LSTM on CEMP dataset
...
if params.model_type=='rnn' then
	params.rnn=nn.Recurrent(params.wdim,nn.LookupTable(#params.index2word,params.wdim),nn.Linear(params.wdim,params.wdim),nn.Sigmoid(),params.rho)
...
params.model:add(params.rnn)
params.model:add(nn.Linear(params.wdim,2))
params.model:add(nn.SoftMax())
